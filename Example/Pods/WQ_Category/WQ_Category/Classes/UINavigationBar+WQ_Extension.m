...
//  ChildrenPalace
//
//  Created by 吴琼 on 2018/12/25.
//  Copyright © 2018 吴琼. All rights reserved.
//

#import "UINavigationBar+WQ_Extension.h"
...
@property (nonatomic, strong) WQ_NavView     *myNavView; //自定义插入层，自定义操作都要在这一层上进行
...
- (void)navBarAlpha:(CGFloat)alpha isOpaque:(BOOL)opaque {
    ...
    if (!self.myNavView) {
        ...
        self.myNavView = [[WQ_NavView alloc]initWithFrame:barBounds];
...
- (void)navBarMyLayerHeight:(CGFloat)height isOpaque:(BOOL)opaque {
    ...
    if (!self.myNavView) {
        self.myNavView = [[WQ_NavView alloc]init];
        [self navBarBottomLineHidden:self.hiddenBottom];
    }
